...
const githubHandlers: Array<RestHandler<MockedRequest<DefaultRequestBody>>> = [
      ...
      const contentDescriptions = await Promise.all(
        dirList.map(async (name): Promise<GHContentsDescription> => {
          ...
          return {
            ...
            sha,
            size,
            url: `https://api.github.com/repos/${owner}/${repo}/content/${path}?${req.url.searchParams}`,
            html_url: `https://github.com/${owner}/${repo}/tree/main/${path}`,
            git_url: `https://api.github.com/repos/${owner}/${repo}/git/trees/${sha}`,
            download_url: null,
            type: isDir ? 'dir' : 'file',
            _links: {
              self: `https://api.github.com/repos/${owner}/${repo}/content/${path}${req.url.searchParams}`,
  ...
  rest.get(
    ...
    async (req, res, ctx) => {
      ...
      // NOTE: we cheat a bit and in the content/:path handler, we set the sha to the relativePath
